...
Section II defines three different scenarios, involving two vessels, and specifies the action to take for both vessel.
The scenarios are a head-on, a crossing and an overtake situation.
A vessel overtaking another vessel shall keep out of the way of the stand on vessel.
A vessel is considered to overtake another vessel when approaching it from a direction more than 22.5 \textdegree abaft the stand on vessels beam, i.e with a relative bearing between 112,5 \textdegree and 247,5 \textdegree from the stand on vessel.
This sector is the same as the the white stern light in  figure \ref{fig:nav_lights} and{} the light red sector in figure \ref{fig:overtake} (\textit{Rule 13}).
...
\begin{figure}[H]
    ...
    \caption{Graphical representations of a FMFs}
...
These values represent the grade to which the given scenario satisfy the antecedents of the rules.
...
Finally the fuzzy sets gained from the previous step need to be converted into a crisp numerical value that can be used by the auto pilot. This process is called defuzzification and is in this case achieved by calculating the geometrical center value along the x axis for the FMFs in figure \ref{fig:ex_cons_tot}.   The results are a +15.5\textdegree course change and a -1,7 knot speed change.
