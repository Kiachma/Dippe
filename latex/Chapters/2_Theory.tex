...
Unmanned surface vehicles (USVs) or autonomous surface crafts (ASCs) are vehicles operating the seas without a crew onboard.
...
COLREGs were originally introduced in 1972, to reduce the number of collisions at sea, and were meant to be interpreted by humans. Situations where multiple rules apply and contradict each other might, therefore, arise. These situations are usually solved with the use of good seamanship and human deduction, which present challenges when translating the COLREGs rules into computer understandable code \cite{benjamin2006method}.
...
%------------------------------------------------------------------------------------
This chapter will present the different parts of COLREGs, with an emphasis on the parts related to manoeuvring a USV in international waters. Information is taken from the official COLREGs \cite{colreg} if not otherwise specified. Part D and E will not be discussed further since they are not of interest for the scope of this thesis. Neither will rules not of interest for the scope of this thesis, from other sections, be mentioned.
%------------------------------------------------------------------------------------
...
Section I in part B addresses how vessels shall operate in order to ensure proper situational awareness with emphasis on collision risk detection.
...
Moreover, regarding every target as an isolated situation might give contradictory decisions in situations that include multiple target vessels. A holistic approach to the situation must, therefore, be taken. This is called good seamanship and is one of the greater challenges when developing a COLREGs compliant algorithm. Good seamanship is even more important when taking into account vessels of different kinds, terrain and economics. A light motorboat might for instance gladly give way to a large tanker on a tight schedule even-though the tanker is obliged to give way.

\begin{figure}[H]
    \centering
    \begin{tabular}{cc}

        \subfloat[Overtake situations][Overtake situations]{
            \includegraphics[width=0.5\textwidth,height=0.42\textheight,keepaspectratio]{Figures/overtake.eps}
            \label{fig:overtake}
        } &
        \subfloat[Head on situation][Head on situation]{
            \includegraphics[width=0.5\textwidth,height=0.42\textheight,keepaspectratio]{Figures/head_on.eps}
            \label{fig:head_on}
        }   \\
        \multicolumn{2}{c}{\subfloat[Crossing situations]{
                \includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{Figures/crossing.eps}
                \label{fig:crossing}
            }}
    \end{tabular}
...
\begin{figure}[H]
    \begin{tabular}{cc}
        ...
        \subfloat[Fuzzy membership function for $\protect\fuzzyset{A}=\left \{ \text{'Is old'} \right \}$]{
            \includegraphics[width=.49\textwidth]{../matlab/FMF_ex.eps}
            \label{fig:FMF_ex}
        } &
        \subfloat[Fuzzy membership function for $\protect\fuzzyset{C}=\{\text{'Is young'}\}, \protect\fuzzyset{B}=\{\text{'Is middle-aged'\}}\text{ and }\protect\fuzzyset{A}=\{\text{'Is old'}\}$]{
            \includegraphics[width=.49\textwidth]{../matlab/FMF_ex2.eps}
            \label{fig:FMF_ex2}
        }
    \end{tabular}

    \caption{Graphical representations of FMFs}
    \label{fig:example_fmfs}
\end{figure}
...
Finally, \textbf{implication}  ($\fuzzyset{P}\to \fuzzyset{Q}: x \text{ is } \fuzzyset{A} \text{, then }x \text{ is } \fuzzyset{B}$) is slightly more difficult to define in fuzzy logic. This is because implication can be defined in multiple ways in classical logic, all of which are equivalent. However, these are not equivalent in fuzzy logic since the law of absorption of negation does not hold. Classical material implication would  define it as: $T(\fuzzyset{P}\to \fuzzyset{Q})=T(\fuzzyset{\overline{P}}\lor\fuzzyset{Q})=max(T(\fuzzyset{\overline{P}}),T(\fuzzyset{Q}))$ \cite{oh1987properties}.
...
This section  explains Mamdani's method by going through an example based on the model presented in section \ref{section:model}. The example uses a scenario where two vessels are located in a 2 dimensional 10*10 NM Cartesian coordinate system.\\
...
Mamdani's inference method can then be used on each rule to obtain the consequent value by taking the minimum of the antecedent value, from the previous step, and the consequent. The result is the original consequent FMF with its top cut of at the antecedent value for the rule, calculated in the previous step. The result can be seen in figure \ref{fig:ex_cons_1_course} as the green area under the starboard curve and in figure \ref{fig:ex_cons_1_speed} as the blue area under the \textit{decrease} curve since rule 1 has \textit{change course to starboard} and \textit{decrease speed} as consequents. The same procedure applies to rule two which has \textit{keep course} and \textit{increase speed} as consequents. The coloured areas are therefore under the orange keep curve in figure \ref{fig:ex_cons_2_course} and the green increase curve in figure \ref{fig:ex_cons_2_speed}
...
The result is two consequent curves for each rule, one for course change and one for speed change. These need to be combined into two final curves to be able to calculate the final course and speed corrections. This is achieved by combining each rule consequent by their maximum values. In this case, the green and orange areas in figures \ref{fig:ex_cons_1_course} and \ref{fig:ex_cons_2_course} are combined to generate figure \ref{fig:ex_cons_tot_course}  while the blue and green areas in figures \ref{fig:ex_cons_1_speed} and \ref{fig:ex_cons_2_speed} generate figure \ref{fig:ex_cons_tot_speed}.
...
Finally, the fuzzy sets gained from the previous step need to be converted into a crisp numerical value that can be used by the autopilot. This process is called defuzzification and is in this case achieved by calculating the geometrical centre value along the x-axis for the FMFs in figures \ref{fig:ex_cons_tot_speed} and \ref{fig:ex_cons_tot_course}.   The results are a +\ang{15.5} course change and a -1.7 kts speed change.
